...
import { marketsData as _marketsData, CustomMarket } from '../market-config';
import { BaseNetworkConfig, ExplorerLinkBuilderConfig, ExplorerLinkBuilderProps, MarketDataType, NetworkConfig } from '../types';
...
const ENABLE_TESTNET = process.env.APP_ENV === 'development' || process.env.APP_ENV === 'alpha' || process.env.APP_ENV === 'local';
 ...
 */
export const networkConfigs = Object.keys(_networkConfigs).reduce((acc: { [key: string]: BaseNetworkConfig }, value) => {
 ...
 */
export const marketsData = Object.keys(_marketsData).reduce((acc: { [key: string]: MarketDataType }, value) => {
  const FORK_ENABLED = typeof window !== 'undefined' ? window.localStorage.getItem('forkEnabled') === 'true' : false;
  const FORK_BASE_CHAIN_ID = typeof window !== 'undefined' ? Number(window.localStorage.getItem('forkBaseChainId')) : 1;
