...
export function replaceParts(svg: string, element: ReplaceElement) {
  ...
  const targetClass = element.target === 'eyes' ? 'g.gotchi-eyeColor' : 'g.gotchi-primary-mouth';
  const textnodes = doc.querySelectorAll(targetClass);

  textnodes.forEach(function (txt) {
    txt.innerHTML = element.target === 'eyes' ? eyes[element.replaceSvg] : mouths[element.replaceSvg];
...
export const customiseSvg = (svg: string, options: CustomiseOptions, equipped?: Tuple<number, 16>) => {
  let styledSvg = svg;
  (Object.keys(options) as Array<keyof typeof options>).map((option): string => {
